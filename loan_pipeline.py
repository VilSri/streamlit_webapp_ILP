...
with header_mid:
    st.markdown("<h1 style='font-size: 21px; color: grey; padding-bottom: 5px;'><b>Intelligent Loan Pipeline Management</b></h1>", unsafe_allow_html = True)
st.divider()
...
# Define the metrics
last_finished_milestone_completion = int(filtered_data["Last Finished Milestone"].eq("Application").sum())
last_finished_milestone_cond_approval = int(filtered_data["Last Finished Milestone"].eq("Started").sum())
last_finished_milestone_doc_preparation = int(filtered_data["Last Finished Milestone"].eq("Reconciled").sum())

# Calculate the rate of each loan decision within the filtered data
milestone_date_approval_rate = milestone_date_approval * 100
clear_to_close_applications_rate = clear_to_close_applications * 100
...
with tab1:
    ...
    with total2:
        ...
        clear_to_close_applications_fig = go.Figure(data = [go.Pie(labels = ["Clear to Close Applications", "Milestone Date Approval"], values = clear_to_close_applications_value, hole = 0.7, title = "Clear to Close Application")])
    ...
    with total3:
        last_finished_milestone_approval_value = [last_finished_milestone_completion,last_finished_milestone_cond_approval, last_finished_milestone_doc_preparation]
        last_finished_milestone_fig = go.Figure(data = [go.Pie(labels = ["Completion", "Cond. Approval", "Document Preparation"], values = last_finished_milestone_approval_value, hole = 0.7, title = "Last Finished Milestone")])
        last_finished_milestone_fig.update_traces(hoverinfo = 'label+value', textfont_size = 15, marker = dict(colors = ["DodgerBlue", "MediumBlue", "DarkSlateBlue"]))
        last_finished_milestone_fig.update_layout(showlegend = False)
        st.plotly_chart(last_finished_milestone_fig, use_container_width = True)
        int(last_finished_milestone_doc_preparation)
...
with tab2:
    ...
    cellsytle_jscode_loan_progress = JsCode("""
    function(params) {
        if (params.value.includes("Pending")) {
            return {
                'color': 'black',
                'backgroundColor': 'Yellow'
            }
        }
        else if (params.value.includes("Closed")) {
            return {
                'color': 'black',
    ...
    cellsytle_jscode_document_expiration_alerts = JsCode("""
    function(params) {
        ...
        else if (params.value === "Expiring Soon") {
            return {
                'color': 'black',
                'backgroundColor': 'Yellow'
            }
        }
        else if (params.value === "Not Expired") {
            return {
                'color': 'black',
                'backgroundColor': 'Lime'
            }
    ...
    gb_loan_progress.configure_default_column(min_column_width = 110, resizable = True, filterable = True, sortable = True, editable = False)
    ...
    gb_loan_progress.configure_column(field = "Progress", header_name = "Progress (%)", wrapHeaderText = True, autoHeaderHeight = True)
    gb_loan_progress.configure_column(field = "ExpRate Lock", header_name = "Rate Lock (10%)", cellStyle = cellsytle_jscode_loan_progress, wrapHeaderText = True, autoHeaderHeight = True)
    gb_loan_progress.configure_column(field = "ExpAppraisal", header_name = "Appraisal (20%)", cellStyle = cellsytle_jscode_loan_progress, wrapHeaderText = True, autoHeaderHeight = True)
    gb_loan_progress.configure_column(field = "Exp_Title", header_name = "Title (20%)", cellStyle = cellsytle_jscode_loan_progress, wrapHeaderText = True, autoHeaderHeight = True)
    gb_loan_progress.configure_column(field = "ExpCredit_Exp", header_name = "Credit Exp (15%)", cellStyle = cellsytle_jscode_loan_progress, wrapHeaderText = True, autoHeaderHeight = True)
    gb_loan_progress.configure_column(field = "Exp_VVOE", header_name = "VVOE (5%)", cellStyle = cellsytle_jscode_loan_progress, wrapHeaderText = True, autoHeaderHeight = True)
    gb_loan_progress.configure_column(field = "Exp_HOI", header_name = "HOI (5%)", cellStyle = cellsytle_jscode_loan_progress, wrapHeaderText = True, autoHeaderHeight = True)
    gb_loan_progress.configure_column(field = "Exp_Payoff", header_name = "Payoffs (10%)", cellStyle = cellsytle_jscode_loan_progress, wrapHeaderText = True, autoHeaderHeight = True)
    gb_loan_progress.configure_column(field = "Exp_Income", header_name = "Income Exp (15%)", cellStyle = cellsytle_jscode_loan_progress, wrapHeaderText = True, autoHeaderHeight = True)
    ...
    loan_progress_table = AgGrid(loan_progress_df, gridOptions = grid_options_loan_progress, columns_auto_size_mode = ColumnsAutoSizeMode.FIT_ALL_COLUMNS_TO_VIEW, height = 400, allow_unsafe_jscode = True)
    ...
    gb_document_expiration_alerts.configure_default_column(min_column_width = 110, resizable = True, filterable = True, sortable = True, editable = False)
    ...
    gb_document_expiration_alerts.configure_column(field = "ExpRate Lock1", header_name = "Rate Lock", cellStyle = cellsytle_jscode_document_expiration_alerts, wrapHeaderText = True, autoHeaderHeight = True)
    gb_document_expiration_alerts.configure_column(field = "ExpAppraisal1", header_name = "Appraisal", cellStyle = cellsytle_jscode_document_expiration_alerts, wrapHeaderText = True, autoHeaderHeight = True)
    gb_document_expiration_alerts.configure_column(field = "Exp_Title1", header_name = "Title", cellStyle = cellsytle_jscode_document_expiration_alerts, wrapHeaderText = True, autoHeaderHeight = True)
    gb_document_expiration_alerts.configure_column(field = "ExpCredit_Exp1", header_name = "Credit", cellStyle = cellsytle_jscode_document_expiration_alerts, wrapHeaderText = True, autoHeaderHeight = True)
    gb_document_expiration_alerts.configure_column(field = "Exp_HOI1", header_name = "HOI", cellStyle = cellsytle_jscode_document_expiration_alerts, wrapHeaderText = True, autoHeaderHeight = True)
    gb_document_expiration_alerts.configure_column(field = "Exp_VVOE1", header_name = "VVOE", cellStyle = cellsytle_jscode_document_expiration_alerts, wrapHeaderText = True, autoHeaderHeight = True)
    gb_document_expiration_alerts.configure_column(field = "Exp_Income1", header_name = "Income", cellStyle = cellsytle_jscode_document_expiration_alerts, wrapHeaderText = True, autoHeaderHeight = True)
    gb_document_expiration_alerts.configure_column(field = "Exp_Payoff1", header_name = "Payoff", cellStyle = cellsytle_jscode_document_expiration_alerts, wrapHeaderText = True, autoHeaderHeight = True)
    grid_options_document_expiration_alerts = gb_document_expiration_alerts.build()
    document_expiration_alerts_table = AgGrid(document_expiration_alerts_df, gridOptions = grid_options_document_expiration_alerts, columns_auto_size_mode = ColumnsAutoSizeMode.FIT_ALL_COLUMNS_TO_VIEW, height = 400, allow_unsafe_jscode = True)
