...
last_finished_milestone = ['Approval', 'Clear to Close', 'Completion', 'Cond. Approval', 'Doc Preparation']
...
df["Loan Source"] = np.random.choice(loan_source_list, len(df), p = [0.25, 0.35, 0.4])
df["Loan Type"] = np.random.choice(loan_type_list, len(df), p = [0.35, 0.4, 0.25])
df["Last Finished Milestone"] = np.random.choice(last_finished_milestone, len(df), p = [0.25, 0.1, 0.05, 0.4, 0.2])
